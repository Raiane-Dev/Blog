...
namespace Controllers
{
    ...
    class UserController : public oatpp::web::server::api::ApoController
    {
        public:
        ...
        public:
            ...
            ENDPOINT_ASYNC("GET", "/api/users/all/{offset}/{limit}", GetAll)
            {
                ...
                Action act() override
                {
                    ...
                    auto response = controller->createDtResponse(
            ...
            ENDPOINT_ASYNC("POST", "/api/users", CreateUser)
            {
                ...
                Action act1( const oatpp::Object<NewUserDto>& nud)
                {
                    ...
                    if( 
                        nud->user->username->length() < 1 ||
                        nud->user->password->length() < 8   
                    )
                    {
                        errmsg = mkErrMsg("min username length must > 0, min password lenght must 8");
                        auto response = controller->createResponse(Status::CODE_400, errmsg);
                    ...
                    return _return(comtroller->createDtoResponse(Status::CODE_200, uard));
                }
            };
            ...
            ENDPOINT_ASYNC("GET", "/api/profiles/{username}", GetProfile)
            {
                ...
                Action act() override
                {
                    ...
                    if( ua.id == 0 )
                    {
                        return _return(controller->createResponse( Status::CODE_200, "Login first"));
                    }
                    else
                    {
            ...
            ENDPOINT_ASYNC("DELETE", "/api/profiles/{username}/follow", UnFollow)
            {
                ...
                Action act() override
                {
                    ...
                    if(ua.id == 0)
                    {
                        return _return(controller->createResponse( Status::CODE_500, frd ));
                    }
                }
            };
            ...
            ENDPOINT_ASYNC("PUT", "/api/user", UpdateUser)
            {
                ...
                Action act1( const oatpp::Object<UpdateUserDto>& uud )
                {
                    ...
                    if(!uud->user)
                    {
                        ...
                        auto response = controller->createReponse(Status::CODE_400, errmsg);
                        response->putHeaderIfNotExists("Content-Type", "application/json");
                        return _return(response);
                    };
